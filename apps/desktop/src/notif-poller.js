...
class NotifPoller {
  ...
  start() {
    ...
    // Kjør først om 10 sek (gir tid til at appen rekker å vise tray osv),
    // deretter periodisk
    this.timer = setInterval(() => this.tick(), POLL_INTERVAL_MS);
    setTimeout(() => this.tick(), 10_000);
    console.log('[NotifPoller] startet (60 sek intervall)');
  }
  ...
  async tick() {
    ...
    try {
      ...
      if (!msg.includes('401')) {
        console.warn('[NotifPoller] tick feilet:', msg);
      }
    } finally {
      this.inFlight = false;
    }
